...
class Datastore(BaseDatastore):
    ...
    def search_with_ids(self, query: str, top_k: int = 5) -> List[tuple[str, str]]:
        """
        Search and return (content, source) tuples for metrics calculation.
        This method is backward compatible and doesn't affect existing functionality.
        ...
        Returns:
            List of (content, source) tuples where source is in filename:chunk_number format
        """
        vector = self.get_vector(query)
        results = (
            self.table.search(vector)
        ...
        result_tuples = [(result.get("content"), result.get("source")) for result in results]
